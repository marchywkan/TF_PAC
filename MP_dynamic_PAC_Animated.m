...
% Attempt at 4D visualization
PAC_3D = permute(All_MVL, [2, 3, 1]);  % [low x time x high]

lowfreq = low(1):1:low(2);
highfreq = high(1):hstep:high(2);
timeaxis = (0:size(PAC_3D,2)-1)/Fs;

filename = 'PAC_animation.gif';  % Output GIF file
figure;

for t = 1:20:size(PAC_3D, 2)
    frame = squeeze(PAC_3D(:, t, :));  % [low x high]
    frame_up = imresize(frame', 4, 'bicubic');  % Smoother visuals

    high_interp = linspace(min(highfreq), max(highfreq), size(frame_up, 1));
    low_interp = linspace(min(lowfreq), max(lowfreq), size(frame_up, 2));

    imagesc(low_interp, high_interp, frame_up);
    set(gca, 'YDir', 'normal');
    xlabel('Phase Frequency (Hz)');
    ylabel('Amplitude Frequency (Hz)');
    title(['PAC at t = ', num2str(timeaxis(t), '%.2f'), ' s']);
    colormap(jet); colorbar;
    clim([0, max(PAC_3D(:))]);
    drawnow;

    % === Save figure (with axes/titles/colorbar) ===
    frame_gif = getframe(gcf);              % ✅ full figure including axes
    img = frame2im(frame_gif);
